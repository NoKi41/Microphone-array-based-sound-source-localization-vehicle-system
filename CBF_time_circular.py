...
for SNR in [200]:
    ...
    for i, angle in enumerate(angles):
        ...
        # print(a.shape)
        # print(R.shape)
        p_cbf[i] = np.dot(a.conj().T, np.dot(R, a)).real  # 计算响应值     

    # print('运算时间：', datetime.datetime.now() - t)

    # 归一化,分贝处理
    p_cbf = 20 * np.log10(p_cbf / np.max(p_cbf))
    # 输出最大的M个值对应角度的
    max_index = np.argsort(p_cbf)[-M:]
    print('最大值对应角度：', angles[max_index])
    print('相对误差：', np.abs(angles[max_index] - SOURCES_theta_angle)/360)

    # 绘制空间谱
    plt.figure(figsize=(8, 8))
    plt.plot(angles, p_cbf)
    plt.xlabel('Angle')
    plt.ylabel('dB')
    # 标注SOURCES_theta_angle中的坐标线
    for u in SOURCES_theta_angle:
        plt.axvline(x = u, color='g', linestyle='--')
    plt.title('CBF spatial spectrum (Frequency: {}Hz)'.format(f), fontsize=20)
    plt.show()

    plt.figure(figsize=(8, 8))
    ax = plt.subplot(111, projection='polar')  # 设置子图为极坐标形式
    ax.plot(np.deg2rad(angles), p_cbf.real)  # 将角度转换为弧度并绘制极坐标图
    ax.set_title('CBF spatial spectrum (Polar) (Frequency: {}Hz)'.format(f), fontsize=20)
    # ax.set_rmin(-5)
    plt.show()